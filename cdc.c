...
#define USB_FIFO_SIZE (((CDC_BUFFER_SIZE) * 2) + 2)
#define USB_FIFO_EMPTY(x) (((x)->write == (x)->read))
#define USB_FIFO_FULL(x)  ((((x)->write + 1) % USB_FIFO_SIZE) == (x)->read)

struct usbbuf_t {
	BYTE inBuf[USB_FIFO_SIZE]; //JTR2
	unsigned char read;
	unsigned char write;
} usbbuf;
...
static unsigned char _usbbufavailable(void)
{
	...
	if (usbbuf.read > usbbuf.write) {
		...
		out = USB_FIFO_SIZE - out;
	} else {
		out = usbbuf.write - usbbuf.read;
	}
